...
gulp.task("clean:dist", function() {
  return gulp.src(config.dist).pipe(vinylPaths(del));
});
...
gulp.task("deploy", function() {
  return gulp.src(config.dist).pipe(deploy());
});
...
gulp.task("copy:fonts", function() {
  ...
  return gulp.src(allFile).pipe(gulp.dest(config.dist + "/fonts"));
});
// images copy
gulp.task("copy:images", function() {
  var allFile = [config.bower + "/bxlider-4/dist/images/*"];
  return gulp.src(allFile).pipe(gulp.dest(config.dist + "/images"));
});
